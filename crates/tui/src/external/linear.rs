...
impl LinearClient {
    ...
    /// Fetch all backlog issues assigned to the current user
    ...
    /// Fetch all backlog issues (regardless of assignee)
    pub async fn fetch_all_backlog_issues(&self) -> Result<Vec<LinearIssue>, String> {
        let query = r#"
            query {
        ...
        let result: GraphQLResponse<IssuesData> = response
            .json()
            .await
            .map_err(|e| format!("JSON parse error: {}", e))?;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_fetch_all_backlog_issues() {
    ...
    #[tokio::test]
    async fn test_fetch_assigned_backlog_issues() {
        let Some(api_key) = get_test_api_key() else {
            eprintln!("Skipping test: VIBE_KANBAN_LINEAR_API_KEY not set");
            return;
        };
