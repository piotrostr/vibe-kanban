...
function projectToFormState(project: Project): ProjectFormState {
	return {
		...
		// Note: linear_api_key is not returned from the server for security,
		// so we use empty string to indicate "unchanged" vs null for "cleared"
		linear_api_key: "",
	};
}
		...
		<div className="space-y-6">
			...
			{selectedProject && draft && (
				<>
					...
					<Card>
						<CardHeader>
							<CardTitle>{t("settings.projects.linear.title")}</CardTitle>
							<CardDescription>
								{t("settings.projects.linear.description")}
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4">
							<div className="space-y-2">
								<Label htmlFor="linear-api-key">
									{t("settings.projects.linear.apiKey.label")}
								</Label>
								<Input
									id="linear-api-key"
									type="password"
									value={draft.linear_api_key}
									onChange={(e) =>
										updateDraft({ linear_api_key: e.target.value })
									}
									placeholder={t("settings.projects.linear.apiKey.placeholder")}
									className="font-mono"
								/>
								<p className="text-sm text-muted-foreground">
									{t("settings.projects.linear.apiKey.helper")}
								</p>
							</div>
						</CardContent>
					</Card>
