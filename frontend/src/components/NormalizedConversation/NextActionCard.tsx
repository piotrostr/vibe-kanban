...
import { SetDevScriptDialog } from "@/components/dialogs/projects/SetDevScriptDialog";
...
export function NextActionCard({
	...
	const handleDevServerClick = useCallback(() => {
		if (runningDevServer) {
			stop();
		} else if (projectHasDevScript) {
			start();
		} else if (project) {
			// No dev script - show dialog to configure it
			SetDevScriptDialog.show({
				projectId: project.id,
				projectName: project.name,
			});
		}
	}, [runningDevServer, projectHasDevScript, project, start, stop]);
