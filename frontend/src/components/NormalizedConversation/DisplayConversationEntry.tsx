...
}> = ({ content, executionProcessId, isRunning, exitCode }) => {
			...
			{isRunning && (
				<div className="flex items-center justify-end mb-2">
					<button
						onClick={handleStop}
						disabled={isStopping}
						className="flex items-center gap-1 px-2 py-1 text-xs bg-destructive/10 hover:bg-destructive/20 text-destructive rounded transition-colors disabled:opacity-50"
					>
...
function DisplayConversationEntry({
	...
	// Handle quick command output (custom entry type for !commands)
	if ((entryType as { type: string }).type === "quick_command_output") {
		const quickCmd = entryType as {
			type: string;
		...
		return (
			<QuickCommandOutput
				content={entry.content}
				executionProcessId={quickCmd.execution_process_id}
				isRunning={quickCmd.is_running}
				exitCode={quickCmd.exit_code}
			/>
		);
	}
