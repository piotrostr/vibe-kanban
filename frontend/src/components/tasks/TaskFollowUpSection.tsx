...
export function TaskFollowUpSection({
	...
	const { isSendingFollowUp, followUpError, setFollowUpError, onSendFollowUp } =
		useFollowUpSend({
			...
			onAfterSendCleanup: () => {
				...
				addToHistory(localMessage); // Add to history before clearing
	...
	// History navigation handlers for shell-style Ctrl+P/Ctrl+N
	const handleHistoryPrevious = useCallback(
		(currentValue: string): string | null => {
			const result = goToPrevious(currentValue);
			if (result !== null) {
